...
: SampledObjectData (
	IFN < 0 ? x_rest : x_rest.head(GetVerticesCnt(topo, IFN) * 3),
	density, 2,
	IFN < 0 ? topo : topo.topRows(IFN)
), _x_rest(x_rest), _density(density), _stiffness(stiffness) {
	// save space for future usage
	_face_topo.conservativeResize(topo.rows(), 3);
	MatrixXi edge_topo_tmp;
	GenerateSurfaceTopo2D(topo, edge_topo_tmp);

	const int num_total_edges = edge_topo_tmp.rows();
	_x.conservativeResizeLike(x_rest);
	_v.conservativeResizeLike(x_rest);
	_edge_topo.conservativeResize(edge_topo_tmp.rows(), 2);	
	_rest_length.conservativeResize(num_total_edges);
	...
	_mass.conservativeResize(x_rest.size() / 3);
}
