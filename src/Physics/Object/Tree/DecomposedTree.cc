...
namespace {
    const bool treetrunk_registered = ObjectFactory::Instance()->Register("rigid-decomposed-treetrunk", [](const json& config) {return new DecomposedTreeTrunk(config);});
    const bool leaf_registered = ObjectFactory::Instance()->Register("rigid-decomposed-leaf", [](const json& config) {return new DecomposedLeaf(config);});
}
...
void DecomposedTreeTrunk::Distribute(const Ref<const VectorXd> &a) {
