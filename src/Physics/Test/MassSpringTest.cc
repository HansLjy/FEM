...
struct TestMassSpringData {
	TestMassSpringData(int num_points, int num_edges, double stiffness, const MatrixXi& edge_topo, const VectorXd& rest_length) : _dof(num_points * 3), _num_edges(num_edges), _stiffness(stiffness), _edge_topo(edge_topo), _rest_length(rest_length) {}
...
TestMassSpringData* GenerateTestSrpingMassData(int num_points, int num_edges, double stiffness) {
	...
	return new TestMassSpringData(num_points, num_edges, stiffness, edge_topo, rest_length);
}
