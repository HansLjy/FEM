...
class GridBasedPhysics : public SampledPhysics {
public:
	GridBasedPhysics() = default;
	explicit GridBasedPhysics(const json& config) {}
...
template<class Data> void GridBasedPhysics::SetCoordinate(Data *obj, const Ref<const VectorXd> &x) {
	obj->_x.head(obj->_dof) = x;
	obj->UpdateProxyPosition();
}

template<class Data>
double GridBasedPhysics::GetPotential(const Data *data, const Ref<const VectorXd> &x) const {
...
template<class Data>
VectorXd GridBasedPhysics::GetPotentialGradient(const Data* data, const Ref<const VectorXd> &x) const {
	const int num_points = data->_num_points;
	const int num_edges = data->_num_edges;
	const double stiffness = data->_stiffness;
	const double ret_stiffness = data->_ret_stiffness;
...
template<class Data>
void GridBasedPhysics::GetPotentialHessian(const Data* data, const Ref<const VectorXd> &x, COO &coo, int x_offset, int y_offset) const {
