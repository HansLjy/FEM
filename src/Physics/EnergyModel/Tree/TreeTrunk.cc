...
TreeTrunk::TreeTrunk(double rho, double youngs_module, double radius_max, double radius_min, const VectorXd &x, const Vector3d &root)
                     : SampledObject(x, GenerateMass(x, rho, radius_max, radius_min), 1, GenerateTopo(x.size() / 3)),
                       TreeTrunkShape(radius_max, radius_min),
                       _root(root) {
...
ReducedTreeTrunk::ReducedTreeTrunk(int num_segments, double rho, double youngs_module, double radius_max,
   ...
   : ReducedObject(control_points.segment<9>(3),
                   new TreeTrunk(rho, youngs_module, radius_max, radius_min,
                             GenerateX(control_points, num_segments), root),
...
VectorXd ReducedTreeTrunk::GenerateX(const VectorXd &control_points, int num_segments) {
    return ReducedObjectUtils::BezierCurve::GenerateSamplePoints(control_points, num_segments);
}

SparseMatrixXd ReducedTreeTrunk::GenerateBase(int num_segments) {
    return ReducedObjectUtils::BezierCurve::GenerateBase(num_segments).block(0, 3, 3 * (num_segments + 1), 9);
}

VectorXd ReducedTreeTrunk::GenerateShift(int num_segments, const Eigen::Vector3d &first_control_point) {
    return ReducedObjectUtils::BezierCurve::GenerateBase(num_segments).block(0, 0, 3 * (num_segments + 1), 3) * first_control_point;
}
