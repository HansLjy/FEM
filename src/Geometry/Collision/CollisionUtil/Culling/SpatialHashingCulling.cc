...
void SpatialHashingCulling::GetCCDSet(
    ...
    for (auto obj : objs) {
		...
		for (int face_id = 0; face_id < num_faces; face_id++) {
			...
			for (int candidate_obj_id = 0; candidate_obj_id < num_objs; candidate_obj_id++) {
				for (int candidate_vertex_id = 0; candidate_vertex_id < objs[candidate_obj_id].GetCollisionVertices().rows(); candidate_vertex_id++) {
					if (candidate_obj_id == obj_id && (candidate_vertex_id == topo(0) || candidate_vertex_id == topo(1) || candidate_vertex_id == topo(2))) {
						continue;
					}
					ccd_set.push_back(PrimitivePair{
						CollisionType::kVertexFace,
						candidate_obj_id, obj_id,
						candidate_vertex_id, face_id
					});
				}
			}
		}
		...
		for (int edge_id = 0; edge_id < num_edges; edge_id++) {
			...
			for (int candidate_obj_id = 0; candidate_obj_id < num_objs; candidate_obj_id++) {
				for (int candidate_edge_id = 0; candidate_edge_id < objs[candidate_obj_id].GetCollisionEdgeTopo().rows(); candidate_edge_id++) {
					if (candidate_obj_id > obj_id || (candidate_obj_id == obj_id && candidate_edge_id > edge_id)) {
						continue;
					}
					if (candidate_obj_id == obj_id) {
						RowVector2i other_topo = objs[candidate_obj_id].GetCollisionEdgeTopo().row(candidate_edge_id);
