#include "ReducedBezierCurveCollisionShape.h"

void ReducedBezierCurveCollisionShape::Bind(const Object &obj) {
	// TODO:
}

void ReducedBezierCurveCollisionShape::ComputeCollisionShape(const Ref<const VectorXd> &x) {
	// TODO:
}

Vector3d ReducedBezierCurveCollisionShape::GetCollisionVertexVelocity(const Ref<const VectorXd> &v, int idx) const {
	// TODO:
}

const BlockVector & ReducedBezierCurveCollisionShape::GetVertexDerivative(int idx) const {
	// TODO:
}
