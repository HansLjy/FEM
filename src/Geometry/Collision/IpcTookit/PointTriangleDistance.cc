...
namespace IPC {
        ...
        double g[12])
    {
        ...
        g[2] = t32 * t43 * t45 * 2.0;
        double t45_ = t45 * t43;
        g[3] = -t44 * t46 * (t21 * t32 * 2.0 + t22 * t33 * 2.0) - t45_ * ((t34 + t12 * t22) - t13 * t21) * 2.0;
        double t43_ = t44 * t46;
        g[4] = t43_ * (t20 * t32 * 2.0 - t22 * t34 * 2.0) + t45_ * ((t33 + t11 * t22) - t13 * t20) * 2.0;
        g[5] = t43_ * (t20 * t33 * 2.0 + t21 * t34 * 2.0) - t45_ * ((t32 + t11 * t21) - t12 * t20) * 2.0;
        g[6] = t45_ * (t12 * t19 - t13 * t18) * 2.0 + t43_ * (t18 * t32 * 2.0 + t19 * t33 * 2.0);
        g[7] = t45_ * (t11 * t19 - t13 * t17) * -2.0 - t43_ * (t17 * t32 * 2.0 - t19 * t34 * 2.0);
        g[8] = t45_ * (t11 * t18 - t12 * t17) * 2.0 - t43_ * (t17 * t33 * 2.0 + t18 * t34 * 2.0);
        g[9] = t45_ * (t12 * t16 - t13 * t15) * -2.0 - t43_ * (t15 * t32 * 2.0 + t16 * t33 * 2.0);
        g[10] = t45_ * (t11 * t16 - t13 * t14) * 2.0 + t43_ * (t14 * t32 * 2.0 - t16 * t34 * 2.0);
        g[11] = t45_ * (t11 * t15 - t12 * t14) * -2.0 + t43_ * (t14 * t33 * 2.0 + t15 * t34 * 2.0);
    }
    ...
    void point_triangle_distance_gradient(const Vector3d& p, const Vector3d& t0, const Vector3d& t1, const Vector3d& t2,
        Vector12d& grad)
        {
            g_PT(p[0], p[1], p[2],
            t0[0], t0[1], t0[2],
    ...
    void point_triangle_distance_diff_gradient(const Vector3d& p, const Vector3d& t0, const Vector3d& t1, const Vector3d& t2, const Vector3d& pp, const Vector3d& tp0, const Vector3d& tp1, const Vector3d& tp2,
        Vector<double, 12>& g)
    {
        double t11 = p[0] - t0[0];
        double t12 = p[1] - t0[1];
        ...
        double t14 = t0[0] - t1[0];
        double t15 = t0[1] - t1[1];
        double t16 = t0[2] - t1[2];

        double t17 = t0[0] - t2[0];
        double t18 = t0[1] - t2[1];
        double t19 = t0[2] - t2[2];

        double t20 = t1[0] - t2[0];
        double t21 = t1[1] - t2[1];
        double t22 = t1[2] - t2[2];

        double d_t11 = pp[0] - tp0[0];
        double d_t12 = pp[1] - tp0[1];
        double d_t13 = pp[2] - tp0[2];

        double d_t14 = tp0[0] - tp1[0];
        double d_t15 = tp0[1] - tp1[1];
        double d_t16 = tp0[2] - tp1[2];

        double d_t17 = tp0[0] - tp2[0];
        double d_t18 = tp0[1] - tp2[1];
        double d_t19 = tp0[2] - tp2[2];

        double d_t20 = tp1[0] - tp2[0];
        double d_t21 = tp1[1] - tp2[1];
        double d_t22 = tp1[2] - tp2[2];
        //
        double t32 = t14 * t18 - t15 * t17;
        double t33 = t14 * t19 - t16 * t17;
        double t34 = t15 * t19 - t16 * t18;

        double d_t32 = d_t14 * t18 + t14 * d_t18 - d_t15 * t17 - t15 * d_t17;
        double d_t33 = d_t14 * t19 + t14 * d_t19 - d_t16 * t17 - t16 * d_t17;
        double d_t34 = d_t15 * t19 + t15 * d_t19 - d_t16 * t18 - t16 * d_t18;

        double temp = t32 * t32 + t33 * t33 + t34 * t34;
        double t43 = 1.0 / temp;
        double d_t43 = (2.0 / (temp * temp)) * (t32 * d_t32 + t33 * d_t33 + t34 * d_t34);

        double t45 = t13 * t32 + t11 * t34 - t12 * t33;
        double d_t45 = (d_t13 * t32 + t13 * d_t32) + (d_t11 * t34 + t11 * d_t34) - (d_t12 * t33 + t12 * d_t33);

        double t44 = t43 * t43;
        double d_t44 = 2. * t43 * d_t43;

        double t46 = t45 * t45;
        double d_t46 = 2. * t45 * d_t45;

        double t45_ = t45 * t43;
        double d_t45_ = d_t45 * t43 + t45 * d_t43;

        double t43_ = t44 * t46;
        double d_t43_ = d_t44 * t46 + t44 * d_t46;

        g[0] = (d_t34 * (t43 * t45) + t34 * (d_t43 * t45 + t43 * d_t45)) * 2.0;
        g[1] = (d_t33 * (t43 * t45) + t33 * (d_t43 * t45 + t43 * d_t45)) * -2.0;
        g[2] = (d_t32 * (t43 * t45) + t32 * (d_t43 * t45 + t43 * d_t45)) * 2.0;

        g[3] = 2.0 * (
            ((-d_t44 * t46 - t44 * d_t46) * (t21 * t32 + t22 * t33) + (-t44 * t46) * (d_t21 * t32 + t21 * d_t32 + d_t22 * t33 + t22 * d_t33)) -
            (d_t45_ * (t34 + t12 * t22 - t13 * t21) + t45_ * (d_t34 + d_t12 * t22 + t12 * d_t22 - d_t13 * t21 - t13 * d_t21))
            );

        g[4] = 2.0 * ((d_t43_ * (t20 * t32 - t22 * t34) + t43_ * (d_t20 * t32 + d_t20 * d_t32 - d_t22 * t34 - t22 * d_t34)) +
            d_t45_ * (t33 + t11 * t22 - t13 * t20) + t45_ * (d_t33 + d_t11 * t22 + t11 * d_t22 - d_t13 * t20 - t13 * d_t20));

        g[5] = 2.0 * ((d_t43_ * (t20 * t33 + t21 * t34) + t43_ * (d_t20 * t33 + t20 * d_t33 + d_t21 * t34 + t21 * d_t34)) -
            (d_t45_ * (t32 + t11 * t21 - t12 * t20) + t45_ * (d_t32 + d_t11 * t21 + t11 * d_t21 - d_t12 * t20 - t12 * d_t20)));

        g[6] = 2.0 * ((d_t45_ * (t12 * t19 - t13 * t18) + t45_ * (d_t12 * t19 + t12 * d_t19 - d_t13 * t18 - t13 * d_t18)) +
            (d_t43_ * (t18 * t32 + t19 * t33) + t43_ * (d_t18 * t32 + t18 * d_t32 + d_t19 * t33 + t19 * d_t33)));

        g[7] = -2.0 * ((d_t45_ * (t11 * t19 - t13 * t17) + t45_ * (d_t11 * t19 + t11 * d_t19 - d_t13 * t17 - t13 * d_t17)) +
            (d_t43_ * (t17 * t32 - t19 * t34) + t43_ * (d_t17 * t32 + t17 * d_t32 - d_t19 * t34 - t19 * d_t34)));

        g[8] = 2.0 * ((d_t45_ * (t11 * t18 - t12 * t17) + t45_ * (d_t11 * t18 + t11 * d_t18 - d_t12 * t17 - t12 * d_t17)) -
            (d_t43_ * (t17 * t33 + t18 * t34) + t43_ * (d_t17 * t33 + t17 * d_t33 + d_t18 * t34 + t18 * d_t34)));

        g[9] = -2.0 * ((d_t45_ * (t12 * t16 - t13 * t15) + t45_ * (d_t12 * t16 + t12 * d_t16 - d_t13 * t15 - t13 * d_t15)) +
            (d_t43_ * (t15 * t32 + t16 * t33) + t43_ * (d_t15 * t32 + t15 * d_t32 + d_t16 * t33 + t16 * d_t33)));

        g[10] = 2.0 * ((d_t45_ * (t11 * t16 - t13 * t14) + t45_ * (d_t11 * t16 + t11 * d_t16 - d_t13 * t14 - t13 * d_t14)) +
            (d_t43_ * (t14 * t32 - t16 * t34) + t43_ * (d_t14 * t32 + t14 * d_t32 - d_t16 * t34 - t16 * d_t34)));

        g[11] = -2.0 * ((d_t45_ * (t11 * t15 - t12 * t14) + t45_ * (d_t11 * t15 + t11 * d_t15 - d_t12 * t14 - t12 * d_t14)) -
            (d_t43_ * (t14 * t33 + t15 * t34) + t43_ * (d_t14 * t33 + t14 * d_t33 + d_t15 * t34 + t15 * d_t34)));
    }
