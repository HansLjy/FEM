...
namespace IPC {
    ...
    void g_PE2D(double v01, double v02, double v11, double v12, double v21, double v22, double g[6])
    {
        double t13;
        double t14;
        double t23;
    ...
    void g_PE3D(double v01, double v02, double v03, double v11, double v12, double v13, double v21, double v22, double v23, double g[9])
    {
        double t17;
        double t18;
        double t19;
        ...
        g[5] = -t53 + t42 * (t20 * t45 * 2.0 + t21 * t46 * 2.0);
        g[6] = t51 + t42 * (t18 * t44 * 2.0 + t19 * t45 * 2.0);
        g[7] = t52 - t42 * (t17 * t44 * 2.0 - t19 * t46 * 2.0);
        g[8] = t53 - t42 * (t17 * t45 * 2.0 + t18 * t46 * 2.0);
    }

    void H_PE2D(double v01, double v02, double v11, double v12, double v21, double v22, double H[36])
    {
        double t15;
        double t16;
        double t17;
    ...
    void H_PE3D(double v01, double v02, double v03, double v11, double v12, double v13,
        ...
        Vector9d& g)
    {
        ...
        double t23 = e0[0] - e1[0];
        double t24 = e0[1] - e1[1];
        double t25 = e0[2] - e1[2];

        double d_t17 = pp[0] - ep0[0];
        double d_t18 = pp[1] - ep0[1];
        double d_t19 = pp[2] - ep0[2];

        double d_t20 = pp[0] - ep1[0];
        double d_t21 = pp[1] - ep1[1];
        double d_t22 = pp[2] - ep1[2];

        double d_t23 = ep0[0] - ep1[0];
        double d_t24 = ep0[1] - ep1[1];
        double d_t25 = ep0[2] - ep1[2];
        //
        double temp = t23 * t23 + t24 * t24 + t25 * t25;
        double t42 = 1.0 / (temp);
        double d_t42 = (-1 / (temp * temp)) * 2 * (t23 * d_t23 + t24 * d_t24 + t25 * d_t25);

        double t44 = t17 * t21 - t18 * t20;
        double d_t44 = t17 * d_t21 + d_t17 * t21 - (t18 * d_t20 + d_t18 * t20);

        double t45 = t17 * t22 - t19 * t20;
        double d_t45 = t17 * d_t22 + d_t17 * t22 - (t19 * d_t20 + d_t19 * t20);

        double t46 = t18 * t22 - t19 * t21;
        double d_t46 = t18 * d_t22 + d_t18 * t22 - (t19 * d_t21 + d_t19 * t21);

        double t43 = t42 * t42;
        double d_t43 = 2 * t42 * d_t42;

        double t50 = (t44 * t44 + t45 * t45) + t46 * t46;
        double d_t50 = 2 * (t44 * d_t44 + t45 * d_t45 + t46 * d_t46);

        double t51 = 2.0 * t23 * t43 * t50;
        double d_t51 = 2.0 * (t23 * (t43 * d_t50 + d_t43 * t50) + d_t23 * t43 * t50);

        double t52 = 2.0 * t24 * t43 * t50;
        double d_t52 = 2.0 * (t24 * (t43 * d_t50 + d_t43 * t50) + d_t24 * t43 * t50);

        double t53 = 2.0 * t25 * t43 * t50;
        double d_t53 = 2.0 * (t25 * (t43 * d_t50 + d_t43 * t50) + d_t25 * t43 * t50);

        g[0] = 2.0 * (d_t42 * (t24 * t44 + t25 * t45) + t42 * (t24 * d_t44 + d_t24 * t44 + t25 * d_t45 + d_t25 * t45));
        g[1] = -2.0 * (d_t42 * (t23 * t44 - t25 * t46) + t42 * (t23 * d_t44 + d_t23 * t44 - t25 * d_t46 - d_t25 * t46));
        g[2] = -2.0 * (d_t42 * (t23 * t45 + t24 * t46) + t42 * (t23 * d_t45 + d_t23 * t45 + t24 * d_t46 + d_t24 * t46));
        g[3] = -d_t51 - 2.0 * (d_t42 * (t21 * t44 + t22 * t45) + t42 * (t21 * d_t44 + d_t21 * t44 + t22 * d_t45 + d_t22 * t45));
        g[4] = -d_t52 + 2.0 * (d_t42 * (t20 * t44 - t22 * t46) + t42 * (t20 * d_t44 + d_t20 * t44 - t22 * d_t46 - d_t22 * t46));
        g[5] = -d_t53 + 2.0 * (d_t42 * (t20 * t45 + t21 * t46) + t42 * (t20 * d_t45 + d_t20 * t45 + t21 * d_t46 + d_t21 * t46));
        g[6] = d_t51 + 2.0 * (d_t42 * (t18 * t44 + t19 * t45) + t42 * (t18 * d_t44 + d_t18 * t44 + t19 * d_t45 + d_t19 * t45));
        g[7] = d_t52 - 2.0 * (d_t42 * (t17 * t44 - t19 * t46) + t42 * (t17 * d_t44 + d_t17 * t44 - t19 * d_t46 - d_t19 * t46));
        g[8] = d_t53 - 2.0 * (d_t42 * (t17 * t45 + t18 * t46) + t42 * (t17 * d_t45 + d_t17 * t45 + t18 * d_t46 + d_t18 * t46));
    }


    void point_edge_distance_hessian(const Vector3d& p,
        const Vector3d& e0,
        const Vector3d& e1,
        Matrix9d& hess)
    {
